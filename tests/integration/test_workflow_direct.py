...
if not os.getenv("TAVILY_API_KEY"):
    os.environ["TAVILY_API_KEY"] = "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"
...
print(f"[CONFIG] SERPER_API_KEY: {'SET' if os.getenv('SERPER_API_KEY') else 'NOT SET'}")
print(f"[CONFIG] FIRECRAWL_API_KEY: {'SET' if os.getenv('FIRECRAWL_API_KEY') else 'NOT SET'}")
print(f"[CONFIG] TAVILY_API_KEY: {'SET' if os.getenv('TAVILY_API_KEY') else 'NOT SET'}")
print()
    ...
    with get_db_session() as db:
        ...
        if final_state.get('reverse_search_results'):
            ...
            if isinstance(results, dict):
                # New format with providers dict
                if 'providers' in results:
                    providers = results['providers']
                    print(f"  Total providers: {len(providers)}")
                    print(f"  Providers with results: {results.get('providers_searched', 0)}")
                    ...
                    for provider, data in providers.items():
                        if isinstance(data, dict):
                            ...
                            status = "OK" if count > 0 else "EMPTY" if not error else "ERROR"
                            print(f"  [{status}] {provider}: {count} results")
                            ...
                            # Show sample results
                            if data.get('results') and count > 0:
                                sample = data['results'][0]
                                if 'url' in sample:
                                    print(f"       Sample: {sample.get('title', sample.get('url', ''))[:60]}")
                else:
                    # Old format
